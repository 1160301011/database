...
public class PayphoenFrame extends JFrame {
	/*
	 * 声明私有有组件
	 */	
	...
	private com.shao.model.Bankcard query_paycard_serial;
	private com.shao.model.Bankcard bankcard_query_cardid;
	
//	private com.shao.model.bankcard 
	
	public PayphoenFrame(final String username,final String client_id) {
		...
		JLabel lblNewLabel2 = new JLabel("充值金额：");
				...
				if(!pay_cardField.getText().equals("")){
					if (!pay_moneyField.getText().equals("")){
						...
						if (Float.parseFloat(pay_moneyField.getText()) < 100000) {
							...
							//查询缴费卡号是否存在
							query_paycard = asi.tra_bkd_query(pay_cardField.getText());
							query_paycard_serial = asi.query_paycard_serial(pay_cardField.getText());
							//如果卡中的余额大于所要缴费的余额 进行下一步判断
							if (query_paycard.getCurrent() > Double.parseDouble(pay_moneyField.getText())) {
								DecimalFormat df = new DecimalFormat("0.00 ");
								SimpleDateFormat df1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//设置日期格式
								string=string+df1.format(new Date());
								string=string+"当前程序：生活缴费；责任人：业务员7"+"\n";
								//执行缴费业务扣款
								asi.paybill_trade(query_paycard_serial.getCard_id(),Double.parseDouble(pay_moneyField.getText()));
								try {
									asi.addConsumtion_phone(query_paycard_serial.getCard_serial(),Double.parseDouble(pay_moneyField.getText()));
								...
								string=string+"当前程序：生活缴费；责任人：用户"+"\n";
							...
							string=string+df.format(new Date());
							string=string+"当前程序：生活缴费；责任人：用户"+"\n";
							string=string+"充值金额必须小于10000，请重新输入！"+"\n";
							JOptionPane.showMessageDialog(null, "充值金额必须小于10000，请重新输入！");
							pay_moneyField.setText("");
						} 
						}else{
							SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//设置日期格式
